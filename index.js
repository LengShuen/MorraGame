...
class Player extends React.Component{
    ...
    async seeTotal() {
        const total = await new Promise(resolveTotalP => {
            this.setState({view: 'GetGuess', playable: true, resolveTotalP});
        })
        this.setState({view: 'WaitingForResults', total});
        return total;
    }
    seeOutcome(i){this.setState({view: 'Done', outcome: intToOutcome[i]});}
    seeTotal(total){this.setState({view: 'GetTotal', total: GUESS[total]})}
...
class Deployer extends Player{
    ...
    async deploy() {
        ...
        this.deadline = {ETH: 10, ALGO: 100, CDF: 1000}[reach.connector];
